...
// ========== CLEAN RESPONSE - ONLY @BMW_AURA5 and OSINT ERA ==========
function cleanResponseData(data) {
    if (!data || typeof data !== 'object') return data;
    
    let cleaned = JSON.parse(JSON.stringify(data));
    // Remove any owner/channel fields from external APIs
    const removeFields = ['Developer', 'DM TO BUY ACCESS', 'owner', 'xtradeep', 'Kon_Hu_Mai', 'channel', 'telegram', 'contact', 'instagram', 'twitter', 'fb', 'facebook', 'website', 'github'];
    ...
    // Always add ONLY @BMW_AURA5 and OSINT ERA channel
...
// Endpoints page with working links
app.get('/endpoints', (req, res) => {
    db.all('SELECT * FROM available_apis WHERE is_active = 1', [], (err, apis) => {
        ...
        const formattedApis = (apis || []).map(api => {
            return {
                ...api,
                full_url: `${req.protocol}://${req.get('host')}${api.endpoint}`,
                example_usage: `${req.protocol}://${req.get('host')}${api.endpoint}?key=YOUR_API_KEY&${Object.keys(JSON.parse(api.required_params || '{}'))[0] || 'param'}=value`
            };
        });
...
app.post('/admin/generate-key', requireAuth, (req, res) => {
    ...
    function createKey(apiKey, isCustom) {
        ...
        // Force owner to @BMW_AURA5 and channel to OSINT ERA
        const finalOwner = '@BMW_AURA5';
        const finalChannel = 'https://t.me/OSINTERA_1';
...
app.listen(PORT, () => {
    ...
    console.log('✅ Only @BMW_AURA5 and OSINT ERA channel showing');
