...
db.serialize(() => {
    ...
    db.get(`SELECT COUNT(*) as count FROM available_apis`, [], (err, row) => {
        if (row && row.count === 0) {
            const apis = [
                ['telegram', '📞 Telegram Number Lookup', 'telegram', 'id', '{"id":"7530266953"}', 'Get Telegram account details', '1'],
                ['aadhar_info', '🆔 Aadhar Details', 'aadhar', 'num', '{"num":"652507323571"}', 'Aadhar to personal details', '1'],
                ['family', '👨‍👩‍👧‍👦 Family Details', 'family', 'term', '{"term":"701984830542"}', 'Family lookup from Aadhaar', '1'],
                ['num_india', '🇮🇳 Indian Number Info', 'num-india', 'num', '{"num":"9876543210"}', 'Indian mobile number details', '1'],
                ['num_pak', '🇵🇰 Pakistani Number', 'num-pak', 'number', '{"number":"03001234567"}', 'Pakistani number lookup', '1'],
                ...
                ['vehicle_info', '🚗 Vehicle Full Info', 'vehicle', 'vehicle', '{"vehicle":"UP50P5434"}', 'Vehicle registration details', '1'],
                ['rc_info', '📋 RC Details', 'rc', 'owner', '{"owner":"HR26EV0001"}', 'Registration certificate', '1'],
                ...
                ['snapchat', '👻 Snapchat Profile', 'snapchat', 'username', '{"username":"priyapanchal272"}', 'Snapchat profile info', '1'],
                ['mistral', '🤖 Mistral AI Chat', 'mistral', 'message', '{"message":"Hi"}', 'Chat with Mistral AI', '1'],
                ['aadhaar_family', '👨‍👩‍👧‍👦 Aadhaar Family', 'aadhaar-family', 'id', '{"id":"701984830542"}', 'Complete family details', '3'],
                ...
                ['country_info', '🏳️ Country Info', 'country-info', 'country', '{"country":"india"}', 'Country information', '1'],
                ['search', '🔍 Search Anything', 'search', 'q', '{"q":"era"}', 'DuckDuckGo search', '1'],
                ['ai_image_pro', '🎨 AI Image Pro', 'ai-image-pro', 'prompt', '{"prompt":"sunset"}', 'Advanced AI image gen', '2']
            ];
...
function cleanResponseData(data, endpoint = null) {
    ...
    try {
        ...
        const removeFields = [
            ...
            'by', '@ftgamer2', '@InvalidAyush', 'api_by', '@AMMOL_ZZ', '@CYBERXANMOL'
        ];
            ...
            for (let key in obj) {
                if (obj.hasOwnProperty(key)) {
                    if (removeFields.includes(key) || removeFields.includes(key.toLowerCase())) {
                        ...
                        if (obj[key].includes('AMMOL_ZZ') || obj[key].includes('CYBERXANMOL')) {
                            delete obj[key];
                        }
                    }
...
// ========== MAIN API HANDLER (FIXED - FLEXIBLE ENDPOINT MATCHING) ==========
app.all('/api/:endpoint', globalLimiter, async (req, res) => {
    ...
    db.get('SELECT * FROM api_keys WHERE key = ? AND status = "active"', [userKey], async (err, keyData) => {
        ...
        // ========== SUPER FLEXIBLE ENDPOINT MATCHING ==========
        // Yeh sab cases handle karega:
        // 1. Direct match: pincode -> pincode
        // 2. With _info: pincode -> pincode_info
        // 3. Without _info: pincode_info -> pincode
        // 4. With dash: num-india -> num-india
        // 5. With underscore: num_india -> num-india
        
        let isAllowed = allowedApis.includes('all');
        
        if (!isAllowed) {
            // Direct match
            if (allowedApis.includes(endpoint)) isAllowed = true;
            // Match with _info suffix (pincode -> pincode_info)
            else if (allowedApis.includes(endpoint + '_info')) isAllowed = true;
            // Match without _info suffix (pincode_info -> pincode)
            else if (allowedApis.includes(endpoint.replace(/_info$/, ''))) isAllowed = true;
            // Match with dash (num-india -> num-india)
            else if (allowedApis.includes(endpoint)) isAllowed = true;
            // Match with underscore (num-india -> num_india)
            else if (allowedApis.includes(endpoint.replace(/-/g, '_'))) isAllowed = true;
            // Match without dash (num_india -> num-india)
            else if (allowedApis.includes(endpoint.replace(/_/g, '-'))) isAllowed = true;
        }
        ...
        // Find the correct proxy key
        let proxyKey = endpoint;
        if (!apiProxyMap[proxyKey]) {
            if (apiProxyMap[endpoint + '_info']) proxyKey = endpoint + '_info';
            else if (apiProxyMap[endpoint.replace(/_info$/, '')]) proxyKey = endpoint.replace(/_info$/, '');
            else if (apiProxyMap[endpoint.replace(/-/g, '_')]) proxyKey = endpoint.replace(/-/g, '_');
            else if (apiProxyMap[endpoint.replace(/_/g, '-')]) proxyKey = endpoint.replace(/_/g, '-');
        }
        
        const proxyFn = apiProxyMap[proxyKey];
...
app.listen(PORT, () => {
    ...
    console.log('=====================================');
    console.log('✅ FLEXIBLE ENDPOINT MATCHING ADDED');
    console.log('✅ pincode, ip, sab kaam karenge');
    console.log('✅ Snapchat API included');
