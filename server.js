    ...
    db.get(`SELECT COUNT(*) as count FROM available_apis`, [], (err, row) => {
        if (row && row.count === 0) {
            const apis = [
                ...
                // ========== LEVEL 2: NEW APIs FROM TELEGRAM BOT (8 APIs) ==========
                ['aadhaar_to_pan', '🆚 Aadhaar to PAN', '/api/aadhaar-to-pan', 'aadhar', '{"aadhar":"393933081942"}', 'Get PAN number from Aadhaar card', '2'],
                ['pan_info_new', '🆚 PAN Detailed Info', '/api/pan-info-new', 'pan', '{"pan":"AXDPR2606K"}', 'Get detailed information from PAN card', '2'],
                ['vehicle_address', '🚗 Vehicle to Full Address', '/api/vehicle-address', 'rc', '{"rc":"HR26DA1337"}', 'Get complete address from vehicle number', '2'],
                ['pan_to_gst', '📄 PAN to GST Number', '/api/pan-to-gst', 'pan', '{"pan":"AXDPR2606K"}', 'Get GST number from PAN card', '2'],
                ['vehicle_owner_number', '🚗 Vehicle Owner Mobile', '/api/vehicle-owner-number', 'rc', '{"rc":"HR26DA1337"}', 'Get owner mobile number from vehicle', '2'],
                ['tg_to_number', '🧠 Telegram ID to Mobile', '/api/tg-to-number', 'telegram', '{"telegram":"8489944328"}', 'Get mobile number from Telegram ID', '2'],
                ['number_lookup_tech', '🔍 Advanced Number Lookup', '/api/number-lookup-tech', 'number', '{"number":"9876543210"}', 'Advanced phone number intelligence', '2'],
                ['aadhaar_to_number', '🆚 Aadhaar to Mobile', '/api/aadhaar-to-number', 'number', '{"number":"393933081942"}', 'Get mobile number from Aadhaar', '2'],
                
                // ========== LEVEL 3: FAMILY API ==========
                ['aadhaar_family', '👨‍👩‍👧‍👦 Aadhaar Family Tree', '/api/aadhaar-family', 'term', '{"term":"393933081942"}', 'Complete family details from Aadhaar', '3'],
                
                // ========== LEVEL 4: WEBSITE SCRAPER ==========
                ['website_scraper', '🌐 Website Scraper Tool', '/api/website-scraper', 'url', '{"url":"https://example.com"}', 'Extract data from any website', '4']
            ];
            
            apis.forEach(api => {
                db.run(`INSERT INTO available_apis (name, display_name, endpoint, required_params, example_params, description, level) VALUES (?, ?, ?, ?, ?, ?, ?)`, api);
            });
            console.log('✅ 32 APIs inserted (Level 1:22, Level 2:8, Level 3:1, Level 4:1)');
        }
    });
});
...
// ========== API PROXY MAP (Existing + 8 New APIs) ==========
const apiProxyMap = {
    // ========== LEVEL 1: Existing APIs ==========
...
// ========== GENERATE API KEY (FIXED - No "Endpoint not allowed" error) ==========
app.post('/admin/generate-key', requireAuth, (req, res) => {
    ...
    function createKey(apiKey, isCustom) {
        ...
        // ========== FIXED: ALLOWED APIS HANDLING ==========
        let allowedApisJson = '["all"]';  // Default: ALL APIs allowed
        ...
        console.log('Allowed APIs JSON:', allowedApisJson); // Debug log
...
// ========== MAIN API HANDLER (FIXED - No "Endpoint not allowed" error) ==========
app.all('/api/:endpoint', globalLimiter, async (req, res) => {
    ...
    // ========== FIX: Normalize endpoint names (support both hyphen and underscore) ==========
    ...
    db.get('SELECT * FROM api_keys WHERE key = ? AND status = "active"', [userKey], async (err, keyData) => {
        ...
        // ========== FIX: Check if endpoint is allowed ==========
        let allowedApis = [];
        try { 
        ...
        // Check both original and normalized endpoint names
        ...
        if (!isAllowed) {
            ...
            return res.json({ 
                ...
                tip: 'Create a new key with "All APIs" selected or contact @BMW_AURA5',
                contact: '@BMW_AURA5' 
            });
        }
        ...
        // Try both original and normalized endpoint in proxy map
...
app.listen(PORT, () => {
    ...
    console.log('=====================================');
    console.log('✅ FIXED: "Endpoint not allowed" error');
    console.log('✅ 32 APIs total (Level 1:22, Level 2:8, Level 3:1, Level 4:1)');
