...
db.serialize(() => {
    ...
    // Insert ALL APIs (Existing + New Extracted APIs)
    db.get(`SELECT COUNT(*) as count FROM available_apis`, [], (err, row) => {
        if (row && row.count === 0) {
            const apis = [
                // ========== EXISTING APIs ==========
                ['telegram', '📞 Telegram Number Lookup', '/api/telegram', 'id', '{"id":"8489944328"}', 'Get Telegram account details', '1'],
                ...
                ['num_newinfo', '📱 Number to New Info', '/api/num-newinfo', 'id', '{"id":"8489944328"}', 'Telegram based number info', '1'],
                ['veh_to_num', '🚗 Vehicle to Number', '/api/veh-to-num', 'term', '{"term":"UP50P5434"}', 'Vehicle to mobile number', '1'],
                
                // ========== NEW LEVEL 2 APIs (TechVishalBoss) ==========
                ['aadhaar_to_pan', '🆚 Aadhaar to PAN', '/api/aadhaar-to-pan', 'aadhar', '{"aadhar":"393933081942"}', 'Get PAN from Aadhaar number', '2'],
                ['pan_info_new', '🆚 PAN Detailed Info', '/api/pan-info-new', 'pan', '{"pan":"AXDPR2606K"}', 'Detailed PAN card information', '2'],
                ['vehicle_address', '🚗 Vehicle to Address', '/api/vehicle-address', 'rc', '{"rc":"HR26DA1337"}', 'Get address from vehicle number', '2'],
                ['pan_to_gst', '📄 PAN to GST', '/api/pan-to-gst', 'pan', '{"pan":"AXDPR2606K"}', 'Get GST from PAN number', '2'],
                ['vehicle_owner_number', '🚗 Vehicle Owner Number', '/api/vehicle-owner-number', 'rc', '{"rc":"HR26DA1337"}', 'Get owner mobile from vehicle', '2'],
                ['tg_to_number', '🧠 Telegram ID to Number', '/api/tg-to-number', 'telegram', '{"telegram":"8489944328"}', 'Get mobile from Telegram ID', '2'],
                ['number_lookup_tech', '🔍 Number Lookup Pro', '/api/number-lookup-tech', 'number', '{"number":"9876543210"}', 'Advanced number lookup', '2'],
                ['aadhaar_to_number', '🆚 Aadhaar to Number', '/api/aadhaar-to-number', 'number', '{"number":"393933081942"}', 'Get mobile from Aadhaar', '2'],
                
                // ========== NEW LEVEL 3 APIs (Family) ==========
                ['aadhaar_family', '👨‍👩‍👧‍👦 Aadhaar Family', '/api/aadhaar-family', 'term', '{"term":"393933081942"}', 'Complete family details from Aadhaar', '3'],
                
                // ========== NEW LEVEL 4 APIs (Scraper) ==========
                ['website_scraper', '🌐 Website Scraper', '/api/website-scraper', 'url', '{"url":"https://example.com"}', 'Extract data from any website', '4']
            ];
            
            apis.forEach(api => {
                db.run(`INSERT INTO available_apis (name, display_name, endpoint, required_params, example_params, description, level) VALUES (?, ?, ?, ?, ?, ?, ?)`, api);
            });
            console.log('✅ 30+ APIs inserted (Level 1,2,3,4)');
        }
    });
});
...
// ========== API PROXY MAP (Existing + New Extracted APIs) ==========
const apiProxyMap = {
    // Existing APIs
    ...
    'num_newinfo': (p) => `https://cyber-osint-tg-num.vercel.app/api/tginfo?key=${MASTER_KEYS.rogers}&id=${p.id || p.number || p.term}`,
    'veh_to_num': (p) => `https://surya-veh-num-xmrewqs.ramaxinfo.workers.dev/?term=${p.term || p.vehicle || p.num}`,
    
    // ========== NEW LEVEL 2 APIs (TechVishalBoss) ==========
    'aadhaar_to_pan': (p) => `${TECH_API_BASE}?key=${EXTRACTED_KEYS.tech_api}&service=aadhar_to_pan&aadhar=${p.aadhar}`,
    'pan_info_new': (p) => `${TECH_API_BASE}?key=${EXTRACTED_KEYS.tech_api}&service=pan&pan=${p.pan}`,
    'vehicle_address': (p) => `${TECH_API_BASE}?key=${EXTRACTED_KEYS.tech_api}&service=vehicle&rc=${p.rc}`,
    'pan_to_gst': (p) => `${TECH_API_BASE}?key=${EXTRACTED_KEYS.tech_api}&service=pan_to_gst&pan=${p.pan}`,
    'vehicle_owner_number': (p) => `${TECH_API_BASE}?key=${EXTRACTED_KEYS.tech_api}&service=vehicle_owner_number&rc=${p.rc}`,
    'tg_to_number': (p) => `${TECH_API_BASE}?key=${EXTRACTED_KEYS.tech_api}&service=tg_to_number&telegram=${p.telegram}`,
    'number_lookup_tech': (p) => `${TECH_API_BASE}?key=${EXTRACTED_KEYS.tech_api}&service=number&number=${p.number}`,
    'aadhaar_to_number': (p) => `${TECH_API_BASE}?key=${EXTRACTED_KEYS.tech_api}&service=aadhar_to_number&number=${p.number}`,
    
    // ========== NEW LEVEL 3 API (Family) ==========
    'aadhaar_family': (p) => `https://familyyyy-info.vercel.app/key-api?key=${EXTRACTED_KEYS.family_api_key}&term=${p.term}`,
    
    // ========== NEW LEVEL 4 API (Website Scraper) ==========
    'website_scraper': (p) => {
        let url = p.url;
...
app.all('/api/:endpoint', globalLimiter, async (req, res) => {
    ...
    db.get('SELECT * FROM api_keys WHERE key = ? AND status = "active"', [userKey], async (err, keyData) => {
        ...
        let allowedApis = [];
        try { allowedApis = JSON.parse(keyData.allowed_apis || '["all"]'); } catch(e) { allowedApis = ['all']; }
        
        const isAllowed = allowedApis.includes('all') || allowedApis.includes(endpoint);
        
        if (!isAllowed) {
            db.run(`INSERT INTO analytics (api_key, endpoint, status_code, ip_address, date) VALUES (?, ?, ?, ?, ?)`, [userKey, endpoint, 403, clientIp, today]);
            return res.json({ error: 'Endpoint not allowed for this key', contact: '@BMW_AURA5' });
        }
...
app.listen(PORT, () => {
    ...
    console.log('✅ 30+ APIs including Level 1,2,3,4');
    console.log('✅ Extracted APIs from Telegram bot added');
    console.log('=====================================\n');
});
