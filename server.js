    ...
    db.get(`SELECT COUNT(*) as count FROM available_apis`, [], (err, row) => {
        if (row && row.count === 0) {
            const apis = [
                ...
                ['ip_info', '🌐 IP Geolocation', '/api/ip', 'ip', '{"ip":"8.8.8.8"}', 'IP address location (Old)', '1'],
                ['pincode_info', '📍 Pincode Info', '/api/pincode', 'pin', '{"pin":"110001"}', 'Area details from pincode (Old)', '1'],
                ...
                ['ai_image', '🎨 AI Image Gen', '/api/ai-image', 'prompt', '{"prompt":"cyberpunk cat"}', 'Generate AI images from prompt', '1'],
                ['insta_info', '📸 Instagram Info', '/api/insta', 'username', '{"username":"ankit.vaid"}', 'Instagram profile scraper', '1'],
                ['mistral', '🤖 Mistral AI Chat', '/api/mistral', 'message', '{"message":"What is AI?"}', 'Chat with Mistral AI language model', '1'],
                ...
                // ========== NEW ADVANCED APIS ==========
                ['ip_advanced', '🌍 IP Advanced (ipinfo.io)', '/api/ip-advanced', 'ip', '{"ip":"8.8.8.8"}', 'Advanced IP geolocation with ISP data', '1'],
                ['pincode_advanced', '📍 Pincode Advanced (Postal API)', '/api/pincode-advanced', 'pin', '{"pin":"110001"}', 'Complete post office details from pincode', '1'],
                ['country_info', '🏳️ Country API (RestCountries)', '/api/country-info', 'country', '{"country":"india"}', 'Full country information, flag, currency, language', '1']
            ];
            
            apis.forEach(api => {
                db.run(`INSERT INTO available_apis (name, display_name, endpoint, required_params, example_params, description, level) VALUES (?, ?, ?, ?, ?, ?, ?)`, api);
            });
            console.log('✅ APIs inserted (Removed old ones, Added new advanced APIs)');
        }
    });
});
...
const apiProxyMap = {
    ...
    'family': (p) => `https://aadhar-2-ration.noobgamingv40.workers.dev/api/aadhaar?id=${p.term || p.id}`, // New family API
    ...
    'vehicle_info': (p) => `https://vvvin-ng.vercel.app/lookup?rc=${p.vehicle || p.rc || p.term}`, // Replaced veh_to_num
...
function cleanResponseData(data, endpoint = null) {
    ...
    try {
        ...
        // Remove any field that might contain someone else's credit
        const removeFields = [
                ...
                if (obj.hasOwnProperty(key)) {
                    ...
                    if (removeFields.includes(key) || removeFields.includes(key.toLowerCase())) {
                        ...
                        if (obj[key] && typeof obj[key] === 'string') {
                            if (obj[key].includes('@') && !obj[key].includes('bmw_aura5') && !obj[key].includes('OSINTNXERA')) {
                                delete obj[key];
                            }
                        ...
                        cleanObject(obj[key]);
                        // If object becomes empty after cleaning, delete it
        ...
        // Return basic response if cleaning fails
        return {
            owner: '@bmw_aura5',
...
app.post('/admin/generate-key', requireAuth, (req, res) => {
    ...
    function createKey(apiKey, isCustom) {
        ...
        // FIX: Properly handle "Select All" scenario
        if (allowed_apis) {
            // If allowed_apis is an array (from checkboxes)
            if (Array.isArray(allowed_apis)) {
                // If "all" is selected OR no specific APIs selected (empty array)
            ...
            // If allowed_apis is a string (single value)
            ...
            // If no allowed_apis sent (user clicked "Select All" or submitted empty), default to all
            allowedApisJson = '["all"]';
        }
...
app.all('/api/:endpoint', globalLimiter, async (req, res) => {
    ...
    db.get('SELECT * FROM api_keys WHERE key = ? AND status = "active"', [userKey], async (err, keyData) => {
        ...
        // FIX: Check if endpoint is allowed (handles both 'all' and individual endpoints)
...
app.listen(PORT, () => {
    ...
    console.log('=====================================');
    console.log('✅ ALL SELECT issue FIXED');
    console.log('✅ Family API Replaced');
    console.log('✅ Vehicle API Updated (VVVIN)');
    console.log('✅ 3 New Advanced APIs Added');
    console.log('✅ Credit locked to @bmw_aura5');
    console.log('=====================================\n');
});
