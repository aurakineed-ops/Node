...
// ========== ENDPOINT MAPPING (Request → Database Name) ==========
// This maps all possible request endpoints to their database names
...
// Reverse mapping: Database name → all possible request endpoints
const DB_TO_ENDPOINTS = {};
for (const [reqEndpoint, dbName] of Object.entries(ENDPOINT_ALIASES)) {
    if (!DB_TO_ENDPOINTS[dbName]) DB_TO_ENDPOINTS[dbName] = [];
    if (!DB_TO_ENDPOINTS[dbName].includes(reqEndpoint)) {
        DB_TO_ENDPOINTS[dbName].push(reqEndpoint);
    }
}
...
db.serialize(() => {
    ...
    // Insert ALL APIs - FIXED: endpoint names WITHOUT /api/ prefix
    db.get(`SELECT COUNT(*) as count FROM available_apis`, [], (err, row) => {
        if (row && row.count === 0) {
            const apis = [
                ['telegram', '📞 Telegram Number Lookup', 'telegram', 'id', '{"id":"7530266953"}', 'Get Telegram account details and linked number', '1'],
                ['aadhar_info', '🆔 Aadhar Details', 'aadhar', 'num', '{"num":"652507323571"}', 'Aadhar to personal details (Name, Mobile, Address)', '1'],
                ['family', '👨‍👩‍👧‍👦 Family Details', 'family', 'term', '{"term":"701984830542"}', 'Family relationship lookup from Aadhaar', '1'],
            ...
            console.log('✅ APIs inserted (Snapchat API added)');
        }
    });
});
                ...
                if (obj.hasOwnProperty(key)) {
                    if (removeFields.includes(key) || removeFields.includes(key.toLowerCase())) {
                        ...
                        if (obj[key].includes('techvishalboss') || obj[key].includes('TVB_FULL') || obj[key].includes('InvalidAyush') ||
                            obj[key].includes('AMMOL_ZZ') || obj[key].includes('CYBERXANMOL')) {
                            delete obj[key];
                        }
                    }
...
function isEndpointAllowed(requestedEndpoint, allowedApisList) {
    ...
    // If 'all' is allowed, everything is allowed
    if (allowedApisList.includes('all')) return true;

    // Normalize the requested endpoint (remove /api/ prefix if present, lowercase)
    ...
    // Check if requested endpoint matches any allowed API using aliases
    ...
    for (const allowedApi of allowedApisList) {
        ...
        // Check if both map to same database name
        ...
        // e.g., request "pincode" should match "pincode_info"
        const suffixes = ['_info', '_advanced', '_details', '_india', '_pak', '_family', '_pro'];

        // Check: request + suffix == allowed
        for (const suffix of suffixes) {
            if (normalizedRequest + suffix === allowedLower) return true;
            if (allowedLower + suffix === normalizedRequest) return true;
        }

        // Check hyphen vs underscore variations
        const requestHyphen = normalizedRequest.replace(/_/g, '-');
        const requestUnderscore = normalizedRequest.replace(/-/g, '_');
        const allowedHyphen = allowedLower.replace(/_/g, '-');
        const allowedUnderscore = allowedLower.replace(/-/g, '_');

        if (requestHyphen === allowedLower || requestUnderscore === allowedLower) return true;
        if (normalizedRequest === allowedHyphen || normalizedRequest === allowedUnderscore) return true;

        // Check base name match (without suffixes)
        let requestBase = normalizedRequest;
        let allowedBase = allowedLower;
        for (const suffix of suffixes) {
            requestBase = requestBase.replace(suffix, '');
            allowedBase = allowedBase.replace(suffix, '');
        }
        if (requestBase === allowedBase) return true;
    }
...
// ========== MAIN API HANDLER (FIXED ENDPOINT MATCHING) ==========
app.all('/api/:endpoint', globalLimiter, async (req, res) => {
    ...
    db.get('SELECT * FROM api_keys WHERE key = ? AND status = "active"', [userKey], async (err, keyData) => {
        ...
        // ====== FIXED: Use smart endpoint matching ======
        ...
        // Map endpoint to proxy function key
...
app.listen(PORT, () => {
    ...
    console.log('=====================================');
    console.log('✅ FIXED: Smart endpoint matching for all APIs');
    console.log('✅ Snapchat API Added');
