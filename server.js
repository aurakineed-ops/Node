...
db.serialize(() => {
    ...
    // Insert ALL APIs (UPDATED with Telegram Lookup)
    db.get(`SELECT COUNT(*) as count FROM available_apis`, [], (err, row) => {
        if (row && row.count === 0) {
            ...
            console.log('✅ 27 APIs inserted (including Telegram Lookup)');
        }
    });
});
...
// ========== API PROXY MAP (UPDATED with Telegram Lookup) ==========
const apiProxyMap = {
...
app.listen(PORT, () => {
    ...
    console.log('=====================================');
    console.log('✅ TOTAL 27 ENDPOINTS WORKING');
