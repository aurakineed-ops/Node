...
app.all('/api/:endpoint', limiter, async (req, res) => {
    ...
    db.get('SELECT * FROM api_keys WHERE key = ? AND status = "active"', [userKey], async (err, keyData) => {
        ...
        if (keyData.expires_at && new Date(keyData.expires_at) < new Date()) {
            return res.json({ error: `API key expired on ${new Date(keyData.expires_at).toLocaleDateString()}`, contact: '@BMW_AURA5' });
        }
        ...
        try {
            ...
            // Return only the API response WITHOUT the user's API key
            ...
            if (typeof result === 'object') {
                ...
                // REMOVED: result.api_key_used = userKey;  // API key removed from response
            }
            res.json(result);
