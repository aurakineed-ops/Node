...
db.serialize(() => {
    ...
    // Rate limit tracking table (resets every 24 hours)
...
// ========== RATE LIMIT CHECK FUNCTION (24hr reset) ==========
async function checkRateLimit(apiKey, keyData) {
    // If rate limit is disabled or unlimited_hits is true, no limit
    if (!keyData.rate_limit_enabled || keyData.unlimited_hits === 1) {
    ...
    const count = await getTodayRateLimitCount(apiKey, today);
    
    if (count >= keyData.rate_limit_per_day) {
        const resetTime = new Date();
        resetTime.setDate(resetTime.getDate() + 1);
        resetTime.setHours(0, 0, 0, 0);
        return { 
            allowed: false, 
            reason: `Rate limit exceeded: ${keyData.rate_limit_per_day} requests per day`,
            reset_at: resetTime.toISOString()
        };
    }
    
    // Increment counter
    await incrementRateLimit(apiKey, today);
    
    return { 
        allowed: true, 
        remaining: keyData.rate_limit_per_day - (count + 1),
        total: keyData.rate_limit_per_day
    };
}

function getTodayRateLimitCount(apiKey, date) {
...
function incrementRateLimit(apiKey, date) {
    return new Promise((resolve, reject) => {
        db.run(`INSERT INTO rate_limit_tracking (api_key, date, requests)
                VALUES (?, ?, 1)
...
app.post('/head-admin/update-rate-limit', requireHeadAdmin, (req, res) => {
    const { key_id, rate_limit_enabled, rate_limit_per_day } = req.body;
    
    db.run(`UPDATE api_keys 
            SET rate_limit_enabled = ?, rate_limit_per_day = ?
            WHERE id = ?`,
            [rate_limit_enabled === 'true' ? 1 : 0, rate_limit_per_day || 100, key_id],
            function(err) {
                if (err) {
...
// Generate API Key with Unlimited option
app.post('/admin/generate-key', requireAuth, (req, res) => {
    const { name, owner_username, owner_channel, expiry, unlimited, allowed_apis, custom_key, enable_custom,
            rate_limit_enabled, rate_limit_per_day } = req.body;
    ...
    function createKey() {
        ...
        // If unlimited is checked, disable rate limiting
        const isUnlimited = unlimited === 'true';
        const rateLimitEnabled = isUnlimited ? 0 : (rate_limit_enabled === 'true' ? 1 : 0);
        const rateLimitPerDay = isUnlimited ? 0 : (rate_limit_per_day || 100);
        ...
        db.run(`INSERT INTO api_keys (key, name, owner_username, owner_channel, expires_at, unlimited_hits, allowed_apis, status, is_custom,
                rate_limit_enabled, rate_limit_per_day)
                VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?)`, 
                [apiKey, name, owner_username || '@BMW_AURA5', owner_channel || 'https://t.me/OSINTERA_1', expires_at, 
                 ...
                 rateLimitEnabled, rateLimitPerDay], 
                function(err) {
                    if (err) {
...
// ========== API PROXY HANDLER with RATE LIMIT (24hr reset) ==========
    ...
    db.get('SELECT * FROM api_keys WHERE key = ? AND status = "active"', [userKey], async (err, keyData) => {
        ...
        if (endpoint === 'mistral') {
            ...
            return res.json({
                ...
                rate_limit: keyData.rate_limit_enabled && keyData.unlimited_hits !== 1 ? {
                    enabled: true,
        ...
        try {
            ...
            cleanedResult.rate_limit = keyData.rate_limit_enabled && keyData.unlimited_hits !== 1 ? {
                enabled: true,
                per_day: keyData.rate_limit_per_day,
                remaining: rateLimitCheck.remaining || 0,
                resets_in: '24 hours'
...
// ========== CRON JOBS ==========
cron.schedule('0 0 * * *', () => {
    console.log('🔄 Running daily expiry check and rate limit reset...');
    ...
    // Old records are kept for analytics but not used for rate limiting
...
app.listen(PORT, () => {
    ...
    console.log('📊 Rate limiting: 24-hour auto-reset');
    console.log('✅ Unlimited option available - No rate limits when enabled');
    console.log('=====================================\n');
});
