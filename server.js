...
db.serialize(() => {
    ...
    // Check and create head admin if not exists
    db.get(`SELECT * FROM users WHERE username = 'main'`, [], (err, row) => {
        if (err) {
            console.error('Error checking head admin:', err.message);
        } else if (!row) {
    ...
    db.get(`SELECT * FROM users WHERE username = 'superadmin'`, [], (err, row) => {
        if (err) {
            console.error('Error checking super admin:', err.message);
        } else if (!row) {
...
app.all('/api/:endpoint', globalLimiter, async (req, res) => {
    ...
    db.get('SELECT * FROM api_keys WHERE key = ? AND status = "active"', [userKey], async (err, keyData) => {
        ...
        db.run(`INSERT INTO daily_calls (api_key, date, calls) VALUES (?, ?, 1)`, [userKey, today]);
