...
// FIXED: Generate API Key with correct expiry
...
// ========== API PROXY HANDLER (FIXED EXPIRY CHECK) ==========
app.all('/api/:endpoint', limiter, async (req, res) => {
    ...
    db.get('SELECT * FROM api_keys WHERE key = ? AND status = "active"', [userKey], async (err, keyData) => {
        ...
        // FIXED: Proper expiry check with timestamp comparison
        if (keyData.expires_at) {
            ...
            // Optional: Show remaining days warning
            const daysLeft = Math.ceil((expiryDate.getTime() - currentDate.getTime()) / (1000 * 60 * 60 * 24));
            if (daysLeft <= 3 && daysLeft > 0) {
                console.log(`⚠️ Key ${userKey} expires in ${daysLeft} days`);
            }
        }
        ...
        try {
            ...
            if (typeof result === 'object') {
                result.owner = keyData.owner_username || '@BMW_AURA5 / @BMW_AURA1';
...
// ========== CRON JOBS (FIXED) ==========
// FIXED: Daily expiry check at midnight
cron.schedule('0 0 * * *', () => {
