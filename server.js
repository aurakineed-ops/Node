...
app.post('/admin/generate-key', requireAuth, (req, res) => {
    ...
    function createKey(apiKey, isCustom) {
        ...
        // ========== FIXED: Handle allowed_apis properly ==========
        let allowedApisJson = '["all"]'; // Default to all APIs
        ...
        if (allowed_apis) {
            if (Array.isArray(allowed_apis)) {
                ...
                // Single API selected or 'all' string
                if (allowed_apis === 'all') {
                    allowedApisJson = '["all"]';
                } else {
                    allowedApisJson = JSON.stringify([allowed_apis]);
...
app.all('/api/:endpoint', globalLimiter, async (req, res) => {
    ...
    db.get('SELECT * FROM api_keys WHERE key = ? AND status = "active"', [userKey], async (err, keyData) => {
        ...
        // ========== FIXED: Check if endpoint is allowed ==========
        let allowedApis = [];
        try { 
        ...
        // If allowed_apis contains 'all', allow all endpoints
...
app.listen(PORT, () => {
    ...
    console.log('=====================================');
    console.log('✅ Fixed: API permissions now work properly');
    console.log('✅ "all" allows all endpoints');
    console.log('✅ Specific endpoints work individually');
    console.log('=====================================\n');
});
