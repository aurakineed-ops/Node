...
// ========== DATABASE SETUP ==========
const DB_PATH = path.join(__dirname, 'api_keys.db');
...
async function checkRateLimit(apiKey, keyData) {
    // Agar unlimited hai toh koi limit nahi
    if (keyData.unlimited_hits === 1) {
        return { allowed: true, unlimited: true };
    }
    
    // Agar rate limit disabled hai
...
app.all('/api/:endpoint', globalLimiter, async (req, res) => {
    ...
    db.get('SELECT * FROM api_keys WHERE key = ? AND status = "active"', [userKey], async (err, keyData) => {
        ...
        if (!allowedApis.includes('all') && allowedApis.length > 0 && !allowedApis.includes(endpoint)) {
            return res.json({ error: 'Endpoint not allowed' });
        }
...
app.listen(PORT, () => {
    ...
    console.log('✅ Unlimited option: ON = No limits | OFF = Custom limits');
    console.log('💾 All data saved in SQLite (survives restarts)\n');
});
