...
// FIXED: endpoints.ejs - Added statusMap
app.get('/endpoints', (req, res) => {
    db.all('SELECT * FROM available_apis WHERE is_active = 1', [], (err, apis) => {
        // Create statusMap with all APIs as live by default
        const statusMap = {};
        (apis || []).forEach(api => {
...
// FIXED: docs.ejs - Added statusMap
...
app.post('/admin/generate-key', requireAuth, (req, res) => {
    ...
    const { 
        name, app_name, expiry, limit_type, max_total_hits, daily_limit,
        allowed_apis, custom_key, ip_whitelist
    } = req.body;
    ...
    if (custom_key && custom_key.trim() !== '') {
        ...
        if (apiKey.includes(' ')) {
            return res.status(400).send('Invalid custom key: No spaces allowed');
        }
        if (apiKey.length < 3) {
        ...
        db.get('SELECT key FROM api_keys WHERE key = ?', [apiKey], (err, existing) => {
            if (existing) return res.status(400).send('Key already exists. Choose a different name.');
...
app.all('/api/:endpoint', globalLimiter, async (req, res) => {
    ...
    db.get('SELECT * FROM api_keys WHERE key = ? AND status = "active"', [userKey], async (err, keyData) => {
        ...
        // Normalize endpoint names (handle num-india vs num_india)
        let normalizedEndpoint = endpoint;
        // Convert hyphens to underscores for matching
        if (endpoint === 'num-india') normalizedEndpoint = 'num_india';
        if (endpoint === 'num-pak') normalizedEndpoint = 'num_pak';
        if (endpoint === 'name-details') normalizedEndpoint = 'name_details';
        if (endpoint === 'bank-info') normalizedEndpoint = 'bank_info';
        if (endpoint === 'pan-info') normalizedEndpoint = 'pan_info';
        if (endpoint === 'vehicle-info') normalizedEndpoint = 'vehicle_info';
        if (endpoint === 'rc-info') normalizedEndpoint = 'rc_info';
        if (endpoint === 'ip-info') normalizedEndpoint = 'ip_info';
        if (endpoint === 'pincode-info') normalizedEndpoint = 'pincode_info';
        if (endpoint === 'git-info') normalizedEndpoint = 'git_info';
        if (endpoint === 'bgmi-info') normalizedEndpoint = 'bgmi_info';
        if (endpoint === 'ff-info') normalizedEndpoint = 'ff_info';
        if (endpoint === 'aadhar-info') normalizedEndpoint = 'aadhar_info';
        if (endpoint === 'ai-image') normalizedEndpoint = 'ai_image';
        if (endpoint === 'insta-info') normalizedEndpoint = 'insta_info';
        if (endpoint === 'num-fullinfo') normalizedEndpoint = 'num_fullinfo';
        if (endpoint === 'num-newinfo') normalizedEndpoint = 'num_newinfo';
        if (endpoint === 'veh-to-num') normalizedEndpoint = 'veh_to_num';
        
        const isAllowed = allowedApis.includes('all') || allowedApis.includes(endpoint) || allowedApis.includes(normalizedEndpoint);
        ...
        // Use normalized endpoint for proxy map
        const proxyEndpoint = normalizedEndpoint;
        const proxyFn = apiProxyMap[proxyEndpoint];
        if (!proxyFn) {
...
app.listen(PORT, () => {
    ...
    console.log('=====================================');
    console.log('✅ FIXED: statusMap for endpoints.ejs');
    console.log('✅ FIXED: Endpoint name matching (num-india -> num_india)');
    console.log('=====================================\n');
});
