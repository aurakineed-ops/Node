...
db.serialize(() => {
    ...
    // Insert ALL APIs (ALL Tech Vishal Boss APIs REMOVED)
    db.get(`SELECT COUNT(*) as count FROM available_apis`, [], (err, row) => {
        if (row && row.count === 0) {
            const apis = [
                ...
                ['aadhar_info', '🆔 Aadhar to Ration Info', '/api/aadhar', 'num', '{"num":"701984830542"}', 'Aadhar to ration card details', '1'],
                ...
                // ========== LEVEL 3: Family API (Replaced - No Tech Vishal) ==========
                ['aadhaar_family', '👨‍👩‍👧‍👦 Aadhaar Family Details', '/api/aadhaar-family', 'id', '{"id":"701984830542"}', 'Complete family details from Aadhaar', '3'],
            ...
            console.log('✅ APIs inserted (ALL Tech Vishal Boss APIs REMOVED, Family API Replaced)');
        }
    });
});
...
// ========== API PROXY MAP (ALL Tech Vishal Boss APIs REMOVED, Family API Replaced) ==========
const apiProxyMap = {
    // ========== NEW APIS ==========
    ...
    // ========== LEVEL 3: Family API (Replaced with Aadhaar API) ==========
...
function cleanResponseData(data, endpoint = null) {
    ...
    const removeFields = [
        ...
        'techvishalboss', 'TVB_FULL', 'aadhar_to_pan', 'pan_info_new', 'vehicle_address',
        'pan_to_gst', 'tg_to_number', 'number_lookup_tech', 'aadhaar_to_number',
        'aadhar_ration', 'ration_info', 'lookup.php'
    ];
    ...
    function cleanObject(obj) {
        ...
        for (let key in obj) {
            if (removeFields.includes(key.toLowerCase()) || removeFields.includes(key)) {
                ...
                if (obj[key].includes('techvishalboss') || obj[key].includes('TVB_FULL')) {
                    delete obj[key];
                }
            }
...
app.listen(PORT, () => {
    ...
    console.log('=====================================');
    console.log('✅ ALL Tech Vishal Boss APIs REMOVED');
    console.log('✅ Family API Replaced with Aadhaar API');
    console.log('✅ No external usernames visible');
