...
const EXTRACTED_KEYS = {
    tech_api: 'TVB_FULL_52F4672E',  // Updated key
...
// ========== NEW API ENDPOINTS ==========
const NEW_APIS = {
    telegram_lookup: 'https://bronx-tg-king.vercel.app/tg',
    vehicle_to_number: 'https://vvvin-ng.vercel.app/lookup',
    tg_advanced: 'https://bronx-tg-king.vercel.app/tg',
    tg_old: 'http://Api.subhxcosmo.in/api'
};
    ...
    db.get(`SELECT COUNT(*) as count FROM available_apis`, [], (err, row) => {
        if (row && row.count === 0) {
            const apis = [
                ...
                ['aadhar_info', '🆔 Aadhar Info New', '/api/aadhar', 'num', '{"num":"701984830542"}', 'Aadhar to ration card details', '1'],
                ...
                ['tg_advance', '🧠 TG to Number Advanced', '/api/tg-advance', 'query', '{"query":"7530266953"}', 'Advanced Telegram to number lookup', '1'],
                
                // ========== LEVEL 2: Tech APIs (Updated with new key) ==========
                ['aadhaar_to_pan', '🆚 Aadhaar to PAN', '/api/aadhaar-to-pan', 'aadhar', '{"aadhar":"701984830542"}', 'Get PAN from Aadhaar', '2'],
                ...
                ['aadhaar_to_number', '🆚 Aadhaar to Mobile', '/api/aadhaar-to-number', 'number', '{"number":"701984830542"}', 'Get mobile from Aadhaar', '2'],
                ['aadhar_ration', '📋 Aadhaar to Ration', '/api/aadhar-ration', 'aadhar', '{"aadhar":"701984830542"}', 'Get ration card from Aadhaar', '2'],
                ['ration_info', '🍚 Ration Card Info', '/api/ration-info', 'ration', '{"ration":"123456789"}', 'Ration card details', '2'],
                
                // ========== LEVEL 3: Family API ==========
                ['aadhaar_family', '👨‍👩‍👧‍👦 Aadhaar Family', '/api/aadhaar-family', 'term', '{"term":"701984830542"}', 'Complete family details', '3'],
...
// ========== API PROXY MAP (Updated - Tech API Key Changed, New APIs Added) ==========
const apiProxyMap = {
    // ========== NEW APIS ==========
    'telegram': (p) => `http://Api.subhxcosmo.in/api?key=RACK2&type=tg&term=${p.id || p.term || p.number}`,
    'tg_advance': (p) => `https://bronx-tg-king.vercel.app/tg?key=${MASTER_KEYS.bronx_tg_key}&query=${p.query || p.id || p.term}`,
    'veh_to_num': (p) => `https://vvvin-ng.vercel.app/lookup?rc=${p.term || p.vehicle || p.num}`,
    
    // ========== LEVEL 1: Existing APIs ==========
    'aadhar_info': (p) => `https://aadhar-2-ration.noobgamingv40.workers.dev/api/aadhaar?id=${p.num || p.id}`,
    ...
    'num_newinfo': (p) => `https://bronx-tg-king.vercel.app/tg?key=${MASTER_KEYS.bronx_tg_key}&query=${p.id || p.number || p.term}`,
    ...
    'aadhar-ration': (p) => `${TECH_API_BASE}?key=${EXTRACTED_KEYS.tech_api}&service=aadhar_ration&aadhar=${p.aadhar}`,
    'ration-info': (p) => `${TECH_API_BASE}?key=${EXTRACTED_KEYS.tech_api}&service=ration_info&ration=${p.ration}`,
...
function cleanResponseData(data, endpoint = null) {
    ...
    const removeFields = [
        ...
        'bronx_tg_key', 'BRONXop', 'RACK2', 'RACKSUN'
    ];
...
app.listen(PORT, () => {
    ...
    console.log('   - Tech API Key Updated: TVB_FULL_52F4672E');
    console.log('   - Old Telegram API: http://Api.subhxcosmo.in/api?key=RACK2&type=tg');
    console.log('   - Advanced TG API: https://bronx-tg-king.vercel.app/tg');
    console.log('   - New Aadhaar API: https://aadhar-2-ration.noobgamingv40.workers.dev/api/aadhaar');
    console.log('   - Email to Info REMOVED');
    console.log('   - Vehicle to Owner API REMOVED');
    console.log('   - All Tech API endpoints active with new key');
    console.log('   - Aadhaar to Ration & Ration Info added');
